...
[![Tests](https://img.shields.io/badge/tests-77%20passing-3ddc97.svg)](test/run.mjs)
...
- **The model reads the actual code** — the page's JavaScript is downloaded and sent with the request, so analysis is grounded in what the code does rather than what its filename suggests. A character budget caps how much goes (default 120K, ~30K tokens) and is spread across files rather than spent on the first two, first-party and bundles first, known libraries skipped. Files are chosen from the inventory, not by URL extension, so `app.js.zgz`, `bundle.js.gz` and extensionless endpoints are included. The model is told how many files it did *not* get and why. Turn it off in Options if you only want metadata.
...
test/run.mjs                   77 tests, no dependencies
```
...
```bash
node test/run.mjs          # 77 unit tests
