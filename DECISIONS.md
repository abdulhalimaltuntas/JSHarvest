...
60. Model listesi artik SAGLAYICIDAN indiriliyor (listModels), sabit kodlu tahmin listesi yalnizca siralama tercihi olarak kullaniliyor. Sebep gercek bir hata: OpenRouter anahtariyla kullanici "0x-alpha" modelini istedi, sabit liste ilk sirada anthropic/claude-sonnet-4.5 tuttugu icin o secildi. Liste 24 saat storage.local'de onbelleklenir, "Refresh list" ile zorlanabilir; anahtar degisince otomatik tazelenir.
61. Model secici arama destekli ve ilk 60 sonucu cizer (OpenRouter'da yuzlerce model var; hepsini DOM'a basmak popup'i kilitler). Baglam penceresi ve 1M token fiyati satirda gosterilir — secim korlemesine yapilmasin.
62. Hedefli analizler: 'finding' (tek bulgu) ve 'script' (tek dosya). Script analizinde dosya icerigi hedef sunucudan cekilir — Deep Scan ile ayni etik sinir, README'de belirtildi. Hedefli analizler envanterin dolu olmasini gerektirmez.
63. Kaynak kodu baglami (aiIncludeSources) VARSAYILAN KAPALI: gercek kaynak kodu gonderiliyor, bu ciddi bir gizlilik karari. Icerik depolanmiyor; analiz aninda .map dosyalarindan cekiliyor (kota sismesin). node_modules/vendor yollari eleniyor, en buyuk dosyalar oncelikli (asil is mantigi orada).
64. Kullanici tanimli analizler settings.aiCustomAnalyses icinde [{id,label,instruction}] olarak tutulur ve 'custom:<id>' analiz kimligiyle cozulur; AI sekmesinde kesikli cerceveli butonlar olarak yerlesiklerin yanina eklenir.
65. Sag tik menusu tarayicinin contextMenus API'si yerine popup ici ozel menu — ek izin gerektirmiyor ve gorunume ozel eylem sunabiliyor (scripts: Send to AI / curl / source map; findings: bulguyu sor; sources: yol kopyala).
66. Analiz gecmisi storage.local'de origin bazli, 60 kayit sinirli. Gonderilen BAGLAM saklanmaz (tekrar uretilebilir ve icinde bulgu metinleri gecer); yalnizca cikti, model, saglayici ve etiket tutulur.

67. Angajman oturumlari sekme bazli yakalamayi DEGISTIRMEZ, uzerine projeksiyon ekler: lib/store.js aynen calisir, sekme bir oturuma bagliysa ayni kayitlar ayrica storage.local'deki oturum deposuna islenir. Sicak yolu basit tutar ve oturumu sekmeler+zaman uzerinden birlesim yapar. Alternatif (store.js'i session bazli yeniden yazmak) MV3 uyku/rehydrate mantigini riske atardi.
68. Kapsam (scope) hem angajman hijyeni hem GIZLILIK korumasi: kapsam disi host'lar oturuma hic yazilmaz, kisisel gezinme angajman verisine karismaz. Bos kapsam "her sey" demektir (kullanici henuz daraltmadi). "*.acme.com" alan adinin kendisini de kapsar; "acme.com" tam host eslesmesidir — "evil-acme.com" hicbir kalibda kapsama girmez.
69. Otomatik baglama yalnizca autoAttach acik VE kapsami tanimli oturumlar icin calisir; zaten bagli bir sekmenin bagi kapsam disina cikildiginda KOPARILMAZ (kullanici bilincli olarak bagladi, gezinme onu bozmamali).
70. Auth durumu sekme bazli bir etikettir (storage.session), kullanici elle cevirir. Oturuma islenen her kayit authStates kumesine yazilir; yalnizca 'auth' iceren kayitlar "auth-only" olur. Cerez/oturum ele gecirme YOK — sadece etiketleme, cunku amac yuzey karsilastirmasi.
71. Triyaj angajmandan BAGIMSIZ, origin bazli (triage:<origin>) tutulur. Boylece angajman kurmadan hizli bir bakista da isaretleme yapilabilir; rapor bu isaretleri toplayarak kullanir. Tek mekanizma, UX ucurumu yok.
72. Rapor tek dosyalik HTML: harici script/font/CDN yok, dogrudan acilir ve e-postayla yollanabilir. Tum kullanici verisi esc() ile kacisli gomulur (rapor da bir XSS yuzeyidir; enjeksiyon testle dogrulanir).
73. Oturum kotalari: 8000 kayit / 3000 bulgu / 6000 kaynak. Kota tasmasinda yariya indirip tek sefer daha denenir. storage.local varsayilan kotasi sinirli oldugundan kaynak KODU oturuma yazilmaz (yalnizca yollar).

74. AI calistirmasi popup'tan BACKGROUND'a tasindi (kullanici hatasi: popup kapaninca analiz duruyordu). Popup bir sayfadir; kullanici baska yere tikladigi anda yok edilir ve devam eden fetch iptal olur. Artik popup yalnizca goruntuleyici: background calistirir, akis parcalari broadcast ile yayinlanir, popup acildiginda devam eden calismaya yeniden baglanir. Karsi-argumana (MV3 worker olebilir) cevap: 20 saniyede bir API cagrisiyla bosta kalma sayaci sifirlanir ve worker yine de olurse kayit 'interrupted' isaretlenir — kullaniciya yalan soylenmez.
75. Gecmis kaydi ARTIK ISTEK GONDERILDIGI ANDA yazilir (status: 'running', metin bos), sonuc gelince ayni kayit updateAnalysis ile tamamlanir. Boylece popup kapansa, worker olse, hata alinsa bile "ne sorulmustu" ve varsa kismi cikti kaybolmaz. Yalnizca status='done' kayitlarda bos metin reddedilir.
76. Akis parcalari 140ms'de bir toplu yayinlanir; her chunk icin mesaj atmak popup acikken gereksiz trafik uretiyordu. Popup kapaliyken broadcast zaten sessizce dusuyor, calisma etkilenmiyor.

77. Popup icin "yuklenme duman testi" eklendi. Sebep gercek bir regresyon: bir duzenleme popup.js'ten 16 fonksiyonu sildi, sozdizimi gecerli kaldigi icin `node --check` yakalamadi ve arayuz "Reading collected scripts…" ekraninda kilitlendi. Test popup modulunu DOM/chrome stub'lariyla gercekten yukler ve init() zincirini yurutur; tanimsiz referans varsa duser. Stub bilincli olarak gercekci yanit verir (bir sekme + dolu get-tab-data), aksi halde init erken cikip asil kod yolunu hic calistirmaz — testin hatayi yakaladigi enjeksiyonla dogrulandi.

78. AI'a artik sayfadaki JS'in ICERIGI gonderiliyor (aiIncludeCode, varsayilan ACIK). Kullanici hakli bir sikayetle geldi: model "elimde sadece envanter metadata'si var, innerHTML/eval goremem" diyordu. Alternatif olarak sink taramasini JSHarvest'in yapip yalnizca bulgulari gondermesini onerdim; kullanici dogrudan kodun gitmesini istedi ve karar onun.
79. Kod gonderimi butcelidir: toplam karakter butcesi (varsayilan 120K ~ 30K token), dosya basina sinir (40K) ve oncelik sirasi (first-party > bundle > buyukten kucuge). Bilinen kutuphaneler (jquery, bootstrap, gtag...) elenir — butceyi yer, bilgi vermez. Ucuncu taraf varsayilan olarak disarda; degerlendirilen sey ucuncu tarafin kendisiyse acilir.
80. "DOM sinks" analizi eklendi: sink'ler + taint kaynaklari + ulasilabilir yollar. Prompt iki yonlu dengelenmis: kod yoksa "dosya adindan tahmin yurutme, oyle soyle ve dur"; kod varsa "sink tek basina bulgu degildir, sabitle beslenen sink'i acikca ele".
81. Kod indirme uzun surebildigi icin calisma asamalari (fetching-code -> analysing) broadcast ile popup'a bildirilir; kullanici "kac dosya, kac bin karakter" gorur. Indirme background'da, 4 es zamanli, mevcut fetchTextSafe ile (credentials: omit).

82. Kod hedefi secimindeki URL UZANTI filtresi kaldirildi. Gercek hata: Zimbra kurulumunda 20 script'ten yalnizca 1'i AI'a gidiyordu, cunku `TinyMCE_all.js.zgz` gibi dosyalar `\.js$` desenine uymuyordu. Filtre ayrica gereksizdi — envanterdeki her kayit zaten toplayicilar tarafindan JS olarak siniflandirilmisti (script istegi veya JavaScript MIME). Artik yalnizca sourcemap ve blob/data elenir; `.js.zgz`, `.js.gz` ve uzantisiz JS endpoint'leri dahil.
83. Butce artik dosyalara PAYLASTIRILIYOR: sabit dosya-basi sinir (40K) butcenin tamamini ilk 3 dosyaya yediriyordu. Pay = butce / min(aday, 12), taban 6K, tavan kullanicinin perFile ayari. 20 dosyali gercek senaryoda kapsama 1'den 12'ye cikti.
84. Gzip'i cozulmemis ikili icerik tespit edilip atlaniyor (ilk 2000 karakterde %5'ten fazla yazdirilamaz karakter). Aksi halde `.zgz` dosyalari butceyi cop veriyle harcardi.
85. Model artik neyi GORMEDIGINI biliyor: baglama "20 dosyadan 12'si dahil; 2'si okunamadi, ucuncu taraf haric tutuldu, gerisi butceye sigmadi" satiri yaziliyor ve eksik dosyalari guvenli saymamasi, istedigi dosyalari adiyla belirtmesi soyleniyor.
