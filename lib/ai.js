...
export function selectCodeTargets(entries, opts = {}) {
  ...
  return (entries || [])
    .filter((e) => {
      ...
      if (!JS_PATH_RE.test(e.normalizedUrl || '')) return false;
...
export async function collectScriptCode(entries, opts = {}) {
  ...
  if (targets.length === 0) return { samples: [], fetched: 0, budgetUsed: 0 };
  ...
  const worker = async () => {
    while (!stopped) {
      ...
      const text = await fetchTextSafe(target.normalizedUrl, perFile + 1);
      if (!text) continue;

      const remaining = budget - used;
      if (remaining <= 0) { stopped = true; return; }
      const slice = text.slice(0, Math.min(perFile, remaining));
      used += slice.length;
      samples.push({
  ...
  return { samples, fetched: samples.length, budgetUsed: used };
}

/** Indirilen kodu baglam metnine cevirir. */
export function renderScriptCode(samples) {
  if (!samples || samples.length === 0) return '';
  const L = ['', '===== SCRIPT SOURCE =====',
    `${samples.length} file(s) fetched from the page. Read them directly: this is the actual code that runs.`,
  ...
  if (isFollowUp) {
    ...
    if (settings.aiIncludeCode && data && Array.isArray(data.entries) && data.entries.length) {
      try {
        ...
        const { samples, budgetUsed } = await collectScriptCode(data.entries, {
