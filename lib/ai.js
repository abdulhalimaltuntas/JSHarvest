...
import { JS_PATH_RE } from './classify.js';
...
const SYSTEM_PROMPT = [
  ...
  '- When SCRIPT SOURCE is included, read it and ground every claim in it: quote the exact fragment.',
  '- Minified code still tells you what it does. Do not refuse to analyse it; work through it.',
  '- If something you need is genuinely absent from the data, say what is missing in one line and analyse what is there.',
  '',
...
const ANALYSES = {
  ...
  domxss: {
    label: 'DOM sinks',
    instruction: [
      'Hunt for CLIENT-SIDE INJECTION in the supplied script source. If no source was',
      'supplied, say so in one line and stop — do not speculate from filenames.',
      '',
      '## Sinks — every dangerous write you can find, with the exact fragment quoted:',
      'innerHTML / outerHTML / insertAdjacentHTML / document.write / eval / new Function /',
      'setTimeout with a string / srcdoc / location assignment / jQuery .html() /',
      'dangerouslySetInnerHTML / document.domain.',
      '## Sources — attacker-controllable inputs reaching them: location.hash, location.search,',
      'document.referrer, window.name, postMessage handlers, storage reads, URL params.',
      '## Reachable paths — where a source plausibly flows into a sink. Rate each one and be',
      'honest about how much of the flow you can actually see in the code given.',
      '## Verify — the concrete payload or breakpoint that would prove or kill each candidate.',
      '',
      'A sink alone is not a finding. Say plainly when a sink is fed only by constants.'
    ].join('\n')
  },
...
// ---------------------------------------------------------------------------
// Script ICERIGI baglami — AI dosyalari kendisi okur
// ---------------------------------------------------------------------------

const CODE_FETCH_CONCURRENCY = 4;

/**
 * Metin gercekten metin mi? Sunucu gzip'li bir dosyayi Content-Encoding
 * vermeden servis ederse fetch onu cozmez ve elimize ikili veri gecer.
 */
function looksBinary(text) {
  ...
  for (let i = 0; i < sample.length; i++) {
    ...
    // Yazdirilabilir olmayan ve yaygin bosluk olmayan karakterler
    if (code === 0xFFFD || (code < 9) || (code > 13 && code < 32)) odd++;
  }
  return sample.length > 0 && odd / sample.length > 0.05;
}

/** Bilinen kutuphaneler: analiz degeri dusuk, butceyi yer. */
const SKIP_CODE_RE = /\b(?:jquery|bootstrap|lodash|underscore|moment|polyfill|analytics|gtag|gtm|recaptcha|hotjar|clarity|sentry-?bundle)\b/i;

/**
 * Analiz edilecek dosyalari onceliklendirir.
 * Once first-party, once bundle gorunumlu, sonra buyukten kucuge — asil
 * uygulama mantigi orada.
 */
export function selectCodeTargets(entries, opts = {}) {
  const { thirdParty = false, max = 40 } = opts;
  return (entries || [])
    .filter((e) => {
      if (e.scheme !== 'http' && e.scheme !== 'https') return false;
      if (e.kind === 'sourcemap') return false;
      // Uzanti kontrolu BILINCLI OLARAK YOK: envanterdeki her kayit zaten
      // toplayicilar tarafindan JS olarak siniflandirildi (script istegi ya da
      // JavaScript MIME). URL'e gore yeniden elemek `app.js.zgz` (Zimbra),
      // `bundle.js.gz` ve uzantisiz JS endpoint'lerini disarda birakiyordu —
      // yani tam da en ilginc dosyalari.
      if (!thirdParty && e.party !== 'first') return false;
      if (SKIP_CODE_RE.test(e.normalizedUrl || '')) return false;
      return true;
    })
    .sort((a, b) => {
      if ((a.party === 'first') !== (b.party === 'first')) return a.party === 'first' ? -1 : 1;
      if (Boolean(a.isBundle) !== Boolean(b.isBundle)) return a.isBundle ? -1 : 1;
...
/**
 * Secilen dosyalarin ICERIGINI indirir. Toplam karakter butcesi asilinca durur —
 * bundle'lar megabayt olabilir ve hem baglam siniri hem maliyet gercektir.
 *
 * @returns {Promise<{samples: Array<{url,code,truncated,size}>, fetched:number, budgetUsed:number}>}
 */
export async function collectScriptCode(entries, opts = {}) {
  const budget = Math.max(0, opts.budget ?? 120000);
  const perFile = Math.max(1000, opts.perFile ?? 40000);
  if (budget === 0) return { samples: [], fetched: 0, budgetUsed: 0 };

  const targets = selectCodeTargets(entries, { thirdParty: Boolean(opts.thirdParty) });
  if (targets.length === 0) return { samples: [], fetched: 0, budgetUsed: 0, skipped: 0, candidates: 0 };

  // Butceyi dosyalara PAYLASTIR. Sabit bir dosya-basi sinir, butcenin tamamini
  // ilk iki-uc dosyaya yedirip geri kalan 17 dosyayi hic gostermiyordu. Payi
  // aday sayisina gore kucultuyoruz ama anlamli kalmasi icin bir taban var.
  const MIN_SHARE = 6000;
  const SPREAD_OVER = 12;
  const share = Math.max(
    MIN_SHARE,
    Math.min(perFile, Math.floor(budget / Math.min(targets.length, SPREAD_OVER)))
  );
  ...
  const worker = async () => {
    while (!stopped) {
      ...
      if (used >= budget) { stopped = true; return; }

      const target = targets[index];
      const text = await fetchTextSafe(target.normalizedUrl, share + 1);
      if (!text) { skipped++; continue; }
      // .zgz / .gz gibi dosyalar sunucu Content-Encoding vermezse ikili olarak
      // gelir; butceyi cop veriye harcamayalim.
      if (looksBinary(text)) { skipped++; continue; }

      const remaining = budget - used;
      if (remaining <= 0) { stopped = true; return; }
      const slice = text.slice(0, Math.min(share, remaining));
      used += slice.length;
      samples.push({
      ...
      if (typeof opts.onProgress === 'function') {
        opts.onProgress({ fetched: samples.length, total: targets.length, chars: used });
      }
    }
  };

  const pool = [];
  for (let i = 0; i < Math.min(CODE_FETCH_CONCURRENCY, targets.length); i++) pool.push(worker());
  await Promise.all(pool);

  // Onceligi bozmadan geri sirala (paralel indirme sirayi karistirir).
  const order = new Map(targets.map((t, i) => [t.normalizedUrl, i]));
  samples.sort((a, b) => (order.get(a.url) ?? 0) - (order.get(b.url) ?? 0));
...
/** Indirilen kodu baglam metnine cevirir. */
export function renderScriptCode(samples, stats = {}) {
  if (!samples || samples.length === 0) return '';
  const total = stats.inventory || samples.length;
  const missing = Math.max(0, total - samples.length);

  const L = ['', '===== SCRIPT SOURCE =====',
    `${samples.length} of ${total} inventoried script(s) are included below. Read them directly:`
    + ' this is the actual code that runs.',
    'Minified code is normal here — reason about it anyway, and quote the exact fragment you are judging.'];

  // Modelin neyi GORMEDIGINI bilmesi, gordugu kadarina guvenmesinden daha onemli.
  if (missing > 0) {
    const reasons = [];
    if (stats.skipped) reasons.push(`${stats.skipped} could not be read (fetch failed or binary)`);
    if (stats.thirdPartyExcluded) reasons.push('third-party files were excluded by configuration');
    reasons.push('the rest did not fit the character budget');
    L.push(`${missing} file(s) are NOT included: ${reasons.join('; ')}.`
      + ' Treat their behaviour as unknown rather than assuming it is safe,'
      + ' and name the ones you would want next.');
  }
  for (const s of samples) {
    L.push('', `--- ${s.url} (${s.party === 'first' ? 'first-party' : 'third-party'}${s.truncated ? `, truncated from ${s.size} chars` : ''}) ---`, s.code);
  }
  L.push('', '===== END SCRIPT SOURCE =====');
  return L.join('\n');
}
 ...
 */
export async function runAnalysis({ data, analysis, question, history, target, onDelta, onStage, signal } = {}) {
  ...
  if (isFollowUp) {
    ...
    // Sayfadaki JS dosyalarinin ICERIGI: model dosyalari kendisi okur.
    if (settings.aiIncludeCode && data && Array.isArray(data.entries) && data.entries.length) {
      try {
        if (typeof onStage === 'function') onStage({ stage: 'fetching-code' });
        const { samples, budgetUsed, skipped, candidates } = await collectScriptCode(data.entries, {
          budget: settings.aiCodeBudget,
          perFile: settings.aiCodePerFile,
          thirdParty: settings.aiCodeThirdParty,
          onProgress: (p) => { if (typeof onStage === 'function') onStage({ stage: 'fetching-code', ...p }); }
        });
        contextText += renderScriptCode(samples, {
          inventory: candidates,
          skipped,
          thirdPartyExcluded: !settings.aiCodeThirdParty
        });
        if (typeof onStage === 'function') {
          onStage({ stage: 'analysing', files: samples.length, chars: budgetUsed });
        }
      } catch {
        /* kod indirilemezse envanterle devam edilir */
      }
    } else {
      contextText += '\n\nNOTE: script source was not included in this request'
        + ' (disabled in Options), so judge from the inventory metadata alone and say so.';
    }
